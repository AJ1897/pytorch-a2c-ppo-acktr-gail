...
def get_args():
    ...
    parser.add_argument("--algo", default="a2c", help="algorithm to use: a2c | ppo | acktr | random")
    parser.add_argument("--gail", action="store_true", default=False, help="do imitation learning with gail")
    parser.add_argument(
        "--gail-experts-dir", default="./gail_experts", help="directory that contains expert demonstrations for gail"
    )
    ...
    parser.add_argument(
        "--num-processes", type=int, default=1, help="how many training CPU processes to use (default: 16)"
    )
    ...
    parser.add_argument(
        "--gif-interval", type=int, default=100, help="GIF interval, one GIF per n updates (default: 100)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num-env-steps", type=int, default=10e6, help="number of environment steps to train (default: 10e6)"
    )
    parser.add_argument(
        "--env-name", default="PongNoFrameskip-v4", help="environment to train on (default: PongNoFrameskip-v4)"
    )
    parser.add_argument("--log-dir", default="/tmp/gym/", help="directory to save agent logs (default: /tmp/gym)")
    parser.add_argument("--custom-gym", default="", help="import some dependency package for thew gym env")
    parser.add_argument(
        "--save-dir", default="./trained_models/", help="directory to save agent logs (default: ./trained_models/)"
    )
    parser.add_argument("--no-cuda", action="store_true", default=False, help="disables CUDA training")
    parser.add_argument("--vgg", action="store_true", default=False, help="replaces CNN with VGG16")
    parser.add_argument("--mobilenet", action="store_true", default=False, help="replaces CNN with Mobilenet")
    parser.add_argument("--efficientnet", action="store_true", default=False, help="replaces CNN with Efficientnet")
    parser.add_argument(
    ...
    parser.add_argument(
        "--navi", action="store_true", default=False, help="use the custom dictionary observation from the navi project"
    )
    ...
    parser.add_argument("--coeff_reward_stable", type=float, default=0.5, help="modulate the stable reward")
    parser.add_argument("--coeff_reward_ctrl", type=float, default=0.1, help="modulate the ctrl reward")

    # defining the env, only for logging purposes
    parser.add_argument(
        "--action_type", type=str, default=1, help="(for logging only) relative, absolute or incremental actions"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="(for logging only) smooths the actions over multiple timesteps",
    )
    parser.add_argument(
        "--random_rot", type=float, default=1, help="(for logging only) variance of the initial position's angle"
    )
